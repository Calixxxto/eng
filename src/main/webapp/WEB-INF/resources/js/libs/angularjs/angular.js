...
// with correct but slower alternatives.
    ...
    /**
     * @ngdoc function
     * @name angular.toJsDate
     * @module ng
     * @function
     *
     * @description
     * KAP-1053
     * Converts string date 'yyyy-MM-dd HH:mm:ss.sss' into js Date.
     *
     * @param {string|Date} date string in 'yyyy-MM-dd HH:mm:ss.sss' format.
     * @returns {Date} js Date object.
     */
    var KAP_1053_DATE_STR = /^(\d{4,})-?(\d\d)-?(\d\d)(?:\s(\d\d)(?::?(\d\d)(?::?(\d\d)(?:\.(\d+))?)?))?$/;

    function toJsDate(string) {
        if (isDate(string)) {
            return string;
        }
        var match;
        if (match = string.match(KAP_1053_DATE_STR)) {
            var date = new Date(0);
 ...
 * }
    ...
    var inputType = {
        ...
        // analog of "number", but actually it's a tect input type with no "number" type restrictions
        'floatnumber': numberInputType,
