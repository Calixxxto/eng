 ...
 * ngTable: Table + Angular JS
 *
 * @author Vitalii Savchuk <esvit`66@gmail.com>
 * @url https://github.com/esvit/ng-table/
 * @license New BSD License <http://creativecommons.org/licenses/BSD/>
 */
                ...
                return function (scope, element, attrs) {
                    ...
                    if (!element.hasClass('ng-table')) {
                        scope.templates = {
                            ...
                            paginationTop: (attrs.templatePaginationTop ? attrs.templatePaginationTop : 'ng-table/pager.html')
                        };
                        ...
                        var paginationRowTop = angular.element(document.createElement('tr'))
                                .append(angular.element(document.createElement('td'))
                                    .attr({
                                        'ng-table-pagination': 'params',
                                        'template-url': 'templates.paginationTop',
                                        'colspan': columns.length
                                    })),
                            paginationTemplateTop = angular.element(document.createElement('thead')).append(paginationRowTop);

                        element.find('thead').remove();

                        element.addClass('ng-table')
                            .prepend(headerTemplate)
                            .prepend(paginationTemplateTop)
                            .append(paginationTemplate);

                        $compile(headerTemplate)(scope);
                        $compile(paginationTemplate)(scope);
                        $compile(paginationTemplateTop)(scope);
                    }
                };
            }
        }
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	//$templateCache.put('ng-table/filters/select.html', '<select ng-init="params.filter()[name] = \'\'" ng-options="data.id as data.title for data in column.data" ng-model="params.filter()[name]" ng-show="filter==\'select\'" class="filter filter-select form-control" name="{{column.filterName}}"> </select>');
